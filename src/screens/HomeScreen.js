...
export default function HomeScreen({ navigation }) {
  const { layoutMode } = useContext(AppContext);
...
const filteredHotspots = hotspots
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center bg-background px-5">
        <Text className="text-lg font-bold text-dark">Locaties laden...</Text>
        <Text className="mt-2 text-center text-sm text-muted">
          De hotspots worden opgehaald.
        </Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-background px-5 pt-14">
      <Header
        subtitle="Goedemiddag!"
        title="Vind je volgende bioscoop of theater"
      />
      ...
      <View className="mt-7 mb-3 flex-row items-center justify-between">
        <Text className="text-lg font-extrabold text-dark">Nearby</Text>
